...
def impute_by_cat_var(
    df_train: pd.DataFrame,
    num_var: str,
    cat_var: str,
    strategy: Literal["mean", "median", "mode"]='mode',
    df_impute: pd.DataFrame=None
) -> pd.Series:
    """
    Impute missing values in a numerical variable based on categorical variable groups.

    Parameters:
    - df_train (pd.DataFrame): DataFrame containing the training data.
    - num_var (str): Name of the numerical variable to impute.
    - cat_var (str): Name of the categorical variable used for grouping.
    - strategy (Literal["mean", "median", "mode"], optional): Imputation strategy. Default is 'mode'.
    - df_impute (pd.DataFrame, optional): DataFrame containing the data to impute. If None, imputation is applied to df_train. Default is None.

    Returns:
    - pd.Series: Series containing the imputed values.

    Notes:
    - Avoids modifying the original DataFrames.
    - Calculates values for each category in cat_var in df_train.
    - Available imputation strategies: 'mean', 'median', 'mode'.
    - Imputation is applied to df_impute if provided, otherwise to df_train.
    """
    # Avoid overriding dfs:
    df_train = df_train.copy()
    
    # Values are calculated for each cat in cat_var in the df_train.
    if strategy == 'mean':
        value_by_cat_var = df_train.groupby(cat_var)[num_var].mean()
    elif strategy == 'median':
        value_by_cat_var = df_train.groupby(cat_var)[num_var].median()
    elif strategy == 'mode':
        value_by_cat_var = df_train.groupby(cat_var)[num_var].apply(lambda df: df.mode().iloc[0])
    
    # Apply imputation to df_impute.
    if df_impute is None:
        ser_imputed = df_train.loc[pd.isnull(df_train[num_var]), num_var] = df_train.loc[pd.isnull(df_train[num_var])].apply(lambda row: value_by_cat_var.loc[row[cat_var]], axis=1)
    else:
        df_impute = df_impute.copy()
        ser_imputed = df_impute.loc[pd.isnull(df_impute[num_var]), num_var] = df_impute.loc[pd.isnull(df_impute[num_var])].apply(lambda row: value_by_cat_var.loc[row[cat_var]], axis=1)
...
def classif_report_threshold(predictor, 
    ...
    accuracy = np.round(accuracy_score(y_set, y_hat), n_decimals)
    
    precision_1 = np.round(precision_score(y_set, y_hat), n_decimals)
    recall_1 = np.round(recall_score(y_set, y_hat), n_decimals)
    f1score_1 = np.round(f1_score(y_set, y_hat), n_decimals)
    
    precision_0 = np.round(precision_score(y_set, y_hat, pos_label=0), n_decimals)
    recall_0 = np.round(recall_score(y_set, y_hat, pos_label=0), n_decimals)
    f1score_0 = np.round(f1_score(y_set, y_hat, pos_label=0), n_decimals)
