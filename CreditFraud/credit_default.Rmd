...
library(reticulate)
library(rmdformats)
use_python("C:\\Users\\milow\\miniconda3\\envs\\ds4b\\python.exe")
```

# Business Problem
Customers from a company in the banking sector are experiencing **fraudulent transactions, leading to customer dissatisfaction**. As a result, the bank seeks a machine learning solution to improve transsactional monitoring for activity on their customers' cards.

The bank intends to **deploy a small team of fraud analysts who will review risky transactions and decide whether to approve or block them**. This team can review up to 400 transactions per month, and the model's output (fraud scores) will help the analysts prioritize which transactions to assess.

The bank's objective is to **prevent as much fraudulent value as possible** by working through the alerts provided by the model. They have shared one year of historical transaction data with fraud flags, asking you to build a model that predicts the likelihood of a transaction being flagged as fraud.
...
# Loading Data
```{python loading data}
df = pd.read_csv('transactions_obf.csv')
...
```
- We identify **class imbalance issues in target variable**: 875 fraudulent transactions out of 117,746.
...
```{python univ cat, echo=FALSE}
# Show prop and count for each category (Top 5) in descending order for each categorical variable.
for cat in cat_vars:
...
```
- **posEntryMode** and **merchantCountry** exhibit a few predominant categories that constitute approximately 90% of all observations. I consider **grouping  the remaining infrequent categories together**.
  - posEntryMode: 5 (59%), 81 (30%), 1 (9%)...
  - merchantCountry: 826 (80%), 442 (13%), ...
- **For the other three categorical variables, applying frequency encoding** might be more appropriate. This would account for every category since there  are numerous categories (100+) with similar, small relative proportions.

### Numerical variables
```{python univ num, echo=FALSE}
# Numeric variables.
...
```
- The **distribution of transactionAmount is heavily skewed to the right** as mean (53.67) > median (20.25). This indicates the presence of many large transaction amounts.
- Interestingly, there are 183 negative transactions. These **negative amounts do not appear to be recording errors** since they all are less than 0.15 currency units and occur frequently. Discounts? Fee adjusments?

## Handling Missing Values
```{python missing values, echo=FALSE}
df.isna()\
  .mean()\
  .mul(100)\
...
## Feature Engineering
```{python basic feat engin, echo=TRUE}
...
## Numerical X vs Y
```{python num EDA, include=FALSE}
# No other way than generate code chunks individually and the copy and paste to rmd...
columns_to_plot = ['availableCash', 'transactionAmount']

for col in columns_to_plot:
    # Generate Quarto code chunk for each column
    quarto_chunk = f"""
```{{python EDA {col}, echo=FALSE}}
fig = my.plot_box_byCat(df, '{col}', 'isFraud', log_y=True)
fig.show()
```"""
    print(quarto_chunk)
```

```{python EDA availableCash, echo=FALSE}
fig = my.plot_box_byCat(df, 'availableCash', 'isFraud', log_y=True)
fig.show()
```

```{python EDA transactionAmount, echo=FALSE}
fig = my.plot_box_byCat(df, 'transactionAmount', 'isFraud', log_y=True)
fig.show()
```
- On average, **fraudulent transactions tend to involve larger amounts than legitimate transactions**, partly due to the presence of many positive outliers, with averages of 115 and 53, respectively. Additionally, **fraudulent transactions typically are more variable and have lower available cash**,  with a average of 4,575 compared to 6640 for legitimate transactions.

## Categorical X vs Y
```{python cat EDA, include=FALSE}
cat_vars_few_cats = [
    'posEntryMode_red', 'merchantCountry_red',
    'year', 'month', 'day'
]
for col in cat_vars_few_cats:
    # Generate Quarto code chunk for each column
    quarto_chunk = f"""
```{{python EDA {col}, echo=FALSE}}
fig = my.plot_freq_barh_byCat(df['isFraud'], df['{col}'])
fig.show()
```"""
    print(quarto_chunk)
```

```{python EDA posEntryMode_red, echo=FALSE}
fig = my.plot_freq_barh_byCat(df['isFraud'], df['posEntryMode_red'])
fig.show()
```

```{python EDA merchantCountry_red, echo=FALSE}
fig = my.plot_freq_barh_byCat(df['isFraud'], df['merchantCountry_red'])
fig.show()
```

```{python EDA year, echo=FALSE}
fig = my.plot_freq_barh_byCat(df['isFraud'], df['year'])
fig.show()
```

```{python EDA month, echo=FALSE}
fig = my.plot_freq_barh_byCat(df['isFraud'], df['month'])
fig.show()
```

```{python EDA day, echo=FALSE}
fig = my.plot_freq_barh_byCat(df['isFraud'], df['day'])
fig.show()
```
- In terms of categorical variables, fraudulent transactions are more frequently associated with **posEntryMode values of 1 and 81**, and occur more often in **merchant countries other than 826 and 442**. They are also more likely to take place in the **months of June, September, and October**, during the **days 17-23**.

## Analyzing Temporal Patterns in Fraud Transactions
### Num of Transactions per accountNumber
```{python}
...
fig.show()
```
- Let's compare the median values since both count distributions are right-skewed. On median, **accounts with fraudulent transactions make more transactions** than accounts without, with medians of 101 and 63, respectively.

### Fraud vs Legitimate Transactions
```{python fraud, echo=FALSE}
for i, row in df_accounts_with_fraud_count.sample(1).iterrows():
...
- After observing many accounts with fraudulent transactions over time (time series of transaction amounts), I noticed a plethora of diversity in their patterns:
    - **Several fraudulent transactions occur within a span of or hours, minutos or even seconds on the same day**, while some happen on different days.
    ...
    - Fraudulent transactions can **have amounts similar to previous legitimate ones**, while others have unusually high amounts.
    - Several fraudulent transactions on same merchant and place.<br><br>
...
df.sort_values(["accountNumber", "transactionTime"]).iloc[:7]
```
...
df_test.shape
```
- **I implied that the client wants to predict transactions in a monthly basis as they review 400 transactions monthly**. That explains why I chose the last month as test set. 
...
```
- I chose **f1 score as evaluation metric** as I'm interested in detecting fraud transactions and care about both precision and recall in the minority class, (y=1).
...
```
<br>
The **top 10 most important features** based on feature importance provided by LGBM are:<br>
  1.tnx_count_account.<br>
  2.tnx_count_daily.<br>
  3.merchantId_freq.<br>
  4.posEntryMode_red_5.<br>
  5.transactionAmount.<br>
  6.availableCash.<br>
  7.tnx_rolling7_max.<br>
  8.tnx_rolling7_avg.<br>
  9.mmc_freq.<br>
  10.tnx_rolling7_min.
